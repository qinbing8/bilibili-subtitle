...
$tailOut = Join-Path $logDir "$timestamp-audio-proxy-tail.out.log"
$tailErr = Join-Path $logDir "$timestamp-audio-proxy-tail.err.log"
...
$verifyScript = Join-Path $PSScriptRoot 'verify-transcription.ps1'
...
if (-not $IncludeLocalProbe) {
  $verifyArgs += '-NoDebugProxy'
}
if ($Diarization) {
  $verifyArgs += '-Diarization'
}
if ($TextPolish) {
  $verifyArgs += '-TextPolish'
}
...
Write-Host "[tail] 预期: tail 抓到阿里云请求序列；默认关闭本地 probe，避免本机请求噪声"
